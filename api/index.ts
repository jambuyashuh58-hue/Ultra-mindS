...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ userId: user.id }, process.env.JWT_SECRET || 'secret', { expiresIn: '30d' });
    res.json({ token, user: { id: user.id, email: user.email, name: user.name, subscriptionStatus: user.subscriptionStatus } });
  } catch (error) {
...
app.get('/api/auth/me', authMiddleware, async (req: any, res) => {
  try {
    ...
    res.json({ id: user.id, email: user.email, name: user.name, subscriptionStatus: user.subscriptionStatus, currentWeek: user.currentWeek, dailyStreak: user.dailyStreak });
  } catch (error) {
...
app.post('/api/mind-sessions', authMiddleware, async (req: any, res) => {
  try {
    ...
    const session = await prisma.mindSession.create({
      data: { userId: req.userId, sessionType: sessionType || 'BREATHING', durationMinutes: durationMinutes || 10, completedMinutes: durationMinutes || 10, cyclesCompleted: cyclesCompleted || 0, binauralBeatsEnabled: binauralBeatsEnabled ?? true, completedAt: new Date() },
    });
    res.json(session);
  } catch (error) {
